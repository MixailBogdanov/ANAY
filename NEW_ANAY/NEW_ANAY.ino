...
void setup() {
  ...
  Serial.begin(9600);
  Serial2.begin(9600);
  Serial2.setTimeout(50); //время задержки опроса пакетов
  ...
  //Инициализация переменных датчиков 
  pinMode(sensorHigh, INPUT); //сигнал прямого вращения привода шатунов
  pinMode(sensorLow, INPUT); //сигнал обратного вращения привода шатунов
  ...
  Serial2.print((String)"Start.n1.val=" + value_speed + char(255) + char(255) + char(255)); //скорость платформы в уд/мин
  Serial2.print((String)"Start.n0.val=" + value_height + char(255) + char(255) + char(255)); //длина протяжки в мм
  Serial2.print((String)"Settings.n2.val=" + value_steps + char(255) + char(255) + char(255)); //количество шагов на миллиметр протяжки
...
void loop() {
  if (Serial2.available()) {  //обработка команд с HMI панели
    char str[30];
    int amount = Serial2.readBytesUntil(';', str, 30);
    str[amount] = NULL;
    ...
    if (command == "SAVE"){
      ...
      Serial2.print((String)"Start.n0.val=" + value_height + char(255) + char(255) + char(255)); //отправляем значения в панель для визуализации примененых параметров
      Serial2.print((String)"Start.n1.val=" + value_speed + char(255) + char(255) + char(255));     
    }
    if (command == "SAVESET"){      
